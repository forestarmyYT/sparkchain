...
class Sparkchain:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}USE ONLY ON YOUR SECOND ACCOUNT
            """
            f"""
        {Fore.MAGENTA + Style.BRIGHT}Auto Ping {Fore.GREEN + Style.BRIGHT}Sparkchain AI - BOT
        """
        f"""
        {Fore.GREEN + Style.BRIGHT}WE COMMITT THAT ALL IS FOR YOU{Fore.YELLOW + Style.BRIGHT} | NO SELLING NO SPAM
        """
        f"""
        {Fore.RED + Style.BRIGHT}This script is not owned by {Fore.CYAN + Style.BRIGHT}ForestArmy,
        ...
        try:
            if use_proxy_choice == 1:
                async with ClientSession(timeout=ClientTimeout(total=30)) as session:
                    async with session.get("https://raw.githubusercontent.com/itsmesatyavir/freeproxy/06e55e34beae73181486fd605877f893bc01001b/freeproxy") as response:
